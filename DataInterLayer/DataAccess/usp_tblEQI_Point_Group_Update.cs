//------------------------------------------------------------------------------
// <autogenerated>
//     This code was generated by a tool.
//     Runtime Version: v2.0.50727
//
//     Changes to this file may cause incorrect behavior and will be lost if 
//     the code is regenerated.
// </autogenerated>
//------------------------------------------------------------------------------

namespace DDYZ.Ensis.DataSource.DataAccess
{
	using System;
	using System.Data;
	using System.Data.SqlClient;
	using System.Data.SqlTypes;
	using System.Xml;
	using System.Reflection;

	public class usp_tblEQI_Point_Group_Update : BaseProcedure
	{
		private SqlParameter parameterold_fldAutoID;

		private SqlParameter parameterold_fldObject;

		private SqlParameter parameterold_fldType;

		private SqlParameter parameterold_fldName;

		private SqlParameter parameterold_fldUserID;

		private SqlParameter parameterold_fldYear;

		private SqlParameter parameterold_fldPointContent;

		private SqlParameter parameternew_fldObject;

		private SqlParameter parameternew_fldType;

		private SqlParameter parameternew_fldName;

		private SqlParameter parameternew_fldUserID;

		private SqlParameter parameternew_fldYear;

		private SqlParameter parameternew_fldPointContent;

		public usp_tblEQI_Point_Group_Update()
		{
			base.InitCommand("usp_tblEQI_Point_Group_Update");
			ConfigParameter();
		}

		public System.Int32 old_fldAutoID
		{
			get
			{
				return (System.Int32)this.parameterold_fldAutoID.Value;
			}
			set
			{
				this.parameterold_fldAutoID.Value=value;
			}
		}

		public System.String old_fldObject
		{
			get
			{
				return (System.String)this.parameterold_fldObject.Value;
			}
			set
			{
				this.parameterold_fldObject.Value=value;
			}
		}

		public System.Int16 old_fldType
		{
			get
			{
				return (System.Int16)this.parameterold_fldType.Value;
			}
			set
			{
				this.parameterold_fldType.Value=value;
			}
		}

		public System.String old_fldName
		{
			get
			{
				return (System.String)this.parameterold_fldName.Value;
			}
			set
			{
				this.parameterold_fldName.Value=value;
			}
		}

		public System.Int32 old_fldUserID
		{
			get
			{
				return (System.Int32)this.parameterold_fldUserID.Value;
			}
			set
			{
				this.parameterold_fldUserID.Value=value;
			}
		}

		public System.Decimal old_fldYear
		{
			get
			{
				return (System.Decimal)this.parameterold_fldYear.Value;
			}
			set
			{
				this.parameterold_fldYear.Value=value;
			}
		}

		public System.String old_fldPointContent
		{
			get
			{
				return (System.String)this.parameterold_fldPointContent.Value;
			}
			set
			{
				this.parameterold_fldPointContent.Value=value;
			}
		}

		public System.String new_fldObject
		{
			get
			{
				return (System.String)this.parameternew_fldObject.Value;
			}
			set
			{
				this.parameternew_fldObject.Value=value;
			}
		}

		public System.Int16 new_fldType
		{
			get
			{
				return (System.Int16)this.parameternew_fldType.Value;
			}
			set
			{
				this.parameternew_fldType.Value=value;
			}
		}

		public System.String new_fldName
		{
			get
			{
				return (System.String)this.parameternew_fldName.Value;
			}
			set
			{
				this.parameternew_fldName.Value=value;
			}
		}

		public System.Int32 new_fldUserID
		{
			get
			{
				return (System.Int32)this.parameternew_fldUserID.Value;
			}
			set
			{
				this.parameternew_fldUserID.Value=value;
			}
		}

		public System.Decimal new_fldYear
		{
			get
			{
				return (System.Decimal)this.parameternew_fldYear.Value;
			}
			set
			{
				this.parameternew_fldYear.Value=value;
			}
		}

		public System.String new_fldPointContent
		{
			get
			{
				return (System.String)this.parameternew_fldPointContent.Value;
			}
			set
			{
				this.parameternew_fldPointContent.Value=value;
			}
		}

		public void ConfigParameter()
		{
			//--------------------------------------------------------
			this.parameterold_fldAutoID=new SqlParameter();
			this.parameterold_fldAutoID.ParameterName="@old_fldAutoID";
			this.parameterold_fldAutoID.SqlDbType=SqlDbType.Int;
			this.parameterold_fldAutoID.Size=4;
			this.parameterold_fldAutoID.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameterold_fldAutoID);
			//--------------------------------------------------------
			this.parameterold_fldObject=new SqlParameter();
			this.parameterold_fldObject.ParameterName="@old_fldObject";
			this.parameterold_fldObject.SqlDbType=SqlDbType.VarChar;
			this.parameterold_fldObject.Size=20;
			this.parameterold_fldObject.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameterold_fldObject);
			//--------------------------------------------------------
			this.parameterold_fldType=new SqlParameter();
			this.parameterold_fldType.ParameterName="@old_fldType";
			this.parameterold_fldType.SqlDbType=SqlDbType.SmallInt;
			this.parameterold_fldType.Size=2;
			this.parameterold_fldType.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameterold_fldType);
			//--------------------------------------------------------
			this.parameterold_fldName=new SqlParameter();
			this.parameterold_fldName.ParameterName="@old_fldName";
			this.parameterold_fldName.SqlDbType=SqlDbType.VarChar;
			this.parameterold_fldName.Size=100;
			this.parameterold_fldName.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameterold_fldName);
			//--------------------------------------------------------
			this.parameterold_fldUserID=new SqlParameter();
			this.parameterold_fldUserID.ParameterName="@old_fldUserID";
			this.parameterold_fldUserID.SqlDbType=SqlDbType.Int;
			this.parameterold_fldUserID.Size=4;
			this.parameterold_fldUserID.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameterold_fldUserID);
			//--------------------------------------------------------
			this.parameterold_fldYear=new SqlParameter();
			this.parameterold_fldYear.ParameterName="@old_fldYear";
			this.parameterold_fldYear.SqlDbType=SqlDbType.Decimal;
			this.parameterold_fldYear.Size=5;
			this.parameterold_fldYear.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameterold_fldYear);
			//--------------------------------------------------------
			this.parameterold_fldPointContent=new SqlParameter();
			this.parameterold_fldPointContent.ParameterName="@old_fldPointContent";
			this.parameterold_fldPointContent.SqlDbType=SqlDbType.VarChar;
			this.parameterold_fldPointContent.Size=-1;
			this.parameterold_fldPointContent.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameterold_fldPointContent);
			//--------------------------------------------------------
			this.parameternew_fldObject=new SqlParameter();
			this.parameternew_fldObject.ParameterName="@new_fldObject";
			this.parameternew_fldObject.SqlDbType=SqlDbType.VarChar;
			this.parameternew_fldObject.Size=20;
			this.parameternew_fldObject.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameternew_fldObject);
			//--------------------------------------------------------
			this.parameternew_fldType=new SqlParameter();
			this.parameternew_fldType.ParameterName="@new_fldType";
			this.parameternew_fldType.SqlDbType=SqlDbType.SmallInt;
			this.parameternew_fldType.Size=2;
			this.parameternew_fldType.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameternew_fldType);
			//--------------------------------------------------------
			this.parameternew_fldName=new SqlParameter();
			this.parameternew_fldName.ParameterName="@new_fldName";
			this.parameternew_fldName.SqlDbType=SqlDbType.VarChar;
			this.parameternew_fldName.Size=100;
			this.parameternew_fldName.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameternew_fldName);
			//--------------------------------------------------------
			this.parameternew_fldUserID=new SqlParameter();
			this.parameternew_fldUserID.ParameterName="@new_fldUserID";
			this.parameternew_fldUserID.SqlDbType=SqlDbType.Int;
			this.parameternew_fldUserID.Size=4;
			this.parameternew_fldUserID.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameternew_fldUserID);
			//--------------------------------------------------------
			this.parameternew_fldYear=new SqlParameter();
			this.parameternew_fldYear.ParameterName="@new_fldYear";
			this.parameternew_fldYear.SqlDbType=SqlDbType.Decimal;
			this.parameternew_fldYear.Size=5;
			this.parameternew_fldYear.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameternew_fldYear);
			//--------------------------------------------------------
			this.parameternew_fldPointContent=new SqlParameter();
			this.parameternew_fldPointContent.ParameterName="@new_fldPointContent";
			this.parameternew_fldPointContent.SqlDbType=SqlDbType.VarChar;
			this.parameternew_fldPointContent.Size=-1;
			this.parameternew_fldPointContent.Direction=ParameterDirection.Input;
			base.m_cmd.Parameters.Add(this.parameternew_fldPointContent);
		}
	}
}
