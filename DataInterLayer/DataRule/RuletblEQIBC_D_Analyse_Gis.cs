...
{
    public class RuletblEQIBC_D_Analyse_Gis : BaseRule
    {
        /// <summary>
        /// 功能描述    ：  取得底栖生物的综合评价[底栖生物断面（Gis）]
        /// 创建者      ：  du
        /// 创建日期    ：  2015-3-30
        /// 修改者      ：
        /// 修改日期    ：
                                            ...
                                            int iStatType, int iSpaceID)
        {
            try
            {
                usp_tblEQIBCD_Report_EcologicalIndex_Gis usp = new usp_tblEQIBCD_Report_EcologicalIndex_Gis();
                usp.fldYear = iYear;
                usp.fldPhase = iPhase;
            ...
            {
                throw new GetByPKException("打开数据库连接失败", "RuletblEQIBC_D_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
            catch (DBQueryException e)
            {
                throw new GetByPKException("执行Sql语句失败", "RuletblEQIBC_D_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
            catch (Exception e)
            {
                throw new GetByPKException(e.Message, "RuletblEQIBC_D_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
        }
