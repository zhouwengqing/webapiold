...
{
    public class RuletblEQIBC_P_Analyse_Gis : BaseRule
    {
        /// <summary>
        /// 功能描述    ：  取得浮游植物的综合评价[浮游植物断面（Gis）]
        /// 创建者      ：  du
        /// 创建日期    ：  2015-3-30
        /// 修改者      ：
        /// 修改日期    ：
                                            ...
                                            int iStatType, int iSpaceID)
        {
            try
            {
                usp_tblEQIbcp_Report_DataStat_Gis usp = new usp_tblEQIbcp_Report_DataStat_Gis();
                usp.fldYear = iYear;
                usp.fldPhase = iPhase;
            ...
            {
                throw new GetByPKException("打开数据库连接失败", "RuletblEQIBC_P_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
            catch (DBQueryException e)
            {
                throw new GetByPKException("执行Sql语句失败", "RuletblEQIBC_P_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
            catch (Exception e)
            {
                throw new GetByPKException(e.Message, "RuletblEQIBC_P_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
        }
