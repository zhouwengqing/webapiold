...
{
    public class RuletblEQICW_R_Report : BaseRule
    {
        #region 通用服务用到的业务逻辑
        ...
        /// <param name="iAppriseID">地表水评价表：--0：数据明细表、1：水质状况评价表、3：达标情况表</param>
        ...
        public DataTable GetEQICWDataInfo(string sTimeType,string sBeginDate, string sEndDate, string sPoint, string sItem, string sStdName = "GB 3838-2002",
             short sStdLevel = 3,  string cDecCarry = "0", int IsYear = 0,  int iPointType = 0, int iAppriseID = 0, int iSpaceID = 0, short sTatType = 0)
        {
            try
            {
                usp_tblEQIW_R_Report_Apprise_V uspAnalyseData = new usp_tblEQIW_R_Report_Apprise_V();
            ...
            {
                throw new GetByPKException("打开数据库连接失败", "RuletblEQICW_R_Report", "GetEQICWDataInfo", "sTimeType:" + sTimeType +
                                            ",sBeginDate:" + sBeginDate + ",sEndDate:" + sEndDate + ",sItem:" + sItem + ",sPoint:" + sPoint + 
                                            ",sRIStdName:" + sStdName + ",sRIStdLevel:" + sStdLevel.ToString());
            }
            catch (DBQueryException e)
            {
                throw new GetByPKException("执行Sql语句失败"+e.Message, "RuletblEQICW_R_Report", "GetEQICWDataInfo", "sTimeType:" + sTimeType +
                                            ",sBeginDate:" + sBeginDate + ",sEndDate:" + sEndDate + ",sItem:" + sItem + ",sPoint:" + sPoint +
            ...
            {
                throw new GetByPKException(e.Message, "RuletblEQICW_R_Report", "GetEQICWDataInfo", "sTimeType:" + sTimeType +
                                            ",sBeginDate:" + sBeginDate + ",sEndDate:" + sEndDate + ",sItem:" + sItem + ",sPoint:" + sPoint + 
