...
namespace DDYZ.Ensis.Rule.BusinessRule.Query
{
    ...
    /// 修改原因    ：
    /// </summary>
    public class EQIBQuerySql : BaseRule
    {
        ...
        public string GetStrSql(Hashtable hasTable, string strUserName, string strCityCode)
        {
            ...
            strBTypeSql = this.QueryBTypeSql(hasTable);
        ...
        /// <param name="strUserName">用户名</param>
        /// <param name="strCityCode">城市代码</param>
        public string GetStrSqlB(Hashtable hasTable, string strUserName, string strCityCode)
        { 
            string strCityPoint;
            string strTimeSql;
            string strItemSql;
            ...
            strCityPoint = this.QueryConditions(hasTable);
            strTimeSql = this.QueryTimeSql(hasTable);
            strItemSql = this.QueryItemSqlB(hasTable);
        ...
        /// </summary>
        /// 
        private string QueryItemSqlB(Hashtable hasTable)
        {
            string strItemSql;
            if (hasTable["Item"].ToString() != "")
            {
                strItemSql = " fldPCode+'.'+fldCTypeCode in(" + hasTable["Item"].ToString() + ")";
                return strItemSql;
            }
            return "";
        }
        ...
        private string QueryItemSql(Hashtable hasTable)
        {
            ...
            if (hasTable["Item"].ToString() != "")
            {
                strItemSql = " fldTypeCode+'.'+fldCTypeCode in(" + hasTable["Item"].ToString() + ")";
                return strItemSql;
            }
            return "";
        }
        ...
        {
            return "fldAutoID desc,fldSTCode asc,fldRCode asc,fldRSCode asc,fldSAMPH asc,fldSAMPR asc,fldRSC asc,fldYear desc," +
                "fldMonth desc,fldDay desc,fldHour desc, fldTypeCode asc,fldCTypeCode asc";
        }
        ...
        /// 修改原因    ：
        /// </summary>
        public string GetTaxisSqlB(Hashtable hasTimeSqlTable)
        {
            return "fldAutoID desc,fldSTCode asc,fldRCode asc,fldRSCode asc,fldSAMPH asc,fldSAMPR asc,fldRSC asc,fldYear desc," +
                "fldMonth desc,fldDay desc,fldHour desc, fldPCode asc,fldCTypeCode asc";
        }
    }
}