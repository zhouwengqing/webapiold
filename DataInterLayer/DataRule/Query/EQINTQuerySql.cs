...
namespace DDYZ.Ensis.Rule.BusinessRule.Query
{
    ...
    /// 修改原因    ：
    /// </summary>
     public class EQINTQuerySql : BaseRule
    {
        ...
        private string QueryConditions(Hashtable hasTable)
        {
            try
            {
                ...
                if (strPointSql != "")
                    strPointSql = "fldSTCode+'.'+fldRDCode in (" + strPointSql + ")";
                return strPointSql;
            }
            catch (Exception e)
            {
        ...
        /// </summary>
         ...
         {
             return "fldAutoID desc,fldSTCode asc,fldRDCODE asc,fldYear desc,fldMonth desc,fldDay desc,fldHour desc ";
         }
    }
}
