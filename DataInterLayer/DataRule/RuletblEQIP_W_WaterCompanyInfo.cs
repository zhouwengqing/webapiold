...
{
    /// <summary>
    /// 功能描述    ：  对表[tblEQIP_W_WaterCompanyInfo]的数据操作
    /// 创建者      ：  Auto Generator
    /// 创建日期    ：  2009-03-27
    /// 修改者      ：
    /// 修改日期    ：
    ...
    { 
        ...
        public DataTable GetCompanyInfoForGis()
        {
            ...
            {
                throw new GetByPKException("打开数据库连接失败", "RuletblEQIP_W_WaterCompanyInfo", "GetCompanyInfoForGis", "");
            }
            catch (DBQueryException e)
            {
                throw new GetByPKException("执行Sql语句失败", "RuletblEQIP_W_WaterCompanyInfo", "GetCompanyInfoForGis", "");
            }
            catch (Exception e)
            {
