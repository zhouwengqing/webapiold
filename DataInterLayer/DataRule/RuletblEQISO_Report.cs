...
{
    public class RuletblEQISO_Report : BaseRule
    {
        #region 通用服务用到的业务逻辑

        /// 功能描述    ：  土壤统计评价(0:原始数据表、1：基本统计表、2：指数统计表、3：项目各级别统计、4：污染指数统计)
        /// 创建者      ：  都玉新
        /// 创建日期    ：  2015-09-28
        /// 修改者      ：
        /// 修改日期    ：
        ...
        /// <param name="iSpaceID">0:原始数据表、1：基本统计表、2：指数统计表、3：项目各级别统计、4：污染指数统计</param>
        /// <param name="sTatType">备用参数，默认值为0，如果有特殊评价要求时使用</param>
        /// <returns>DataTable</returns>
        public DataTable GetEQISODataInfo(string sTimeType, string sBeginDate, string sEndDate, string sPoint, string sRStdName, short sRStdLevel,
            string sItem, int iSpaceID,  string cDecCarry = "0", 
            int iAppriseID = 0, short sTatType = 0)
        {
            try
            {
                usp_tblEQISO_Report_Apprise uspAnalyseData = new usp_tblEQISO_Report_Apprise();
            ...
            {
                throw new GetByPKException("打开数据库连接失败", "RuletblEQISO_Report", "GetEQISODataInfo", "sTimeType:" + sTimeType +
            ...
            {
                throw new GetByPKException("执行Sql语句失败" + e.Message, "RuletblEQISO_Report", "GetEQISODataInfo", "sTimeType:" + sTimeType +
                                            ",sBeginDate:" + sBeginDate + ",sEndDate:" + sEndDate + ",sItem:" + sItem + ",sPoint:" + sPoint +
                                            ",sRIStdName:" + sRStdName + ",sRIStdLevel:" + sRStdLevel.ToString());
            }
            catch (Exception e)
            {
                throw new GetByPKException(e.Message, "RuletblEQISO_Report", "GetEQISODataInfo", "sTimeType:" + sTimeType +
                                            ",sBeginDate:" + sBeginDate + ",sEndDate:" + sEndDate + ",sItem:" + sItem + ",sPoint:" + sPoint +
                                            ",sRIStdName:" + sRStdName + ",sRIStdLevel:" + sRStdLevel.ToString());
            }
        }
