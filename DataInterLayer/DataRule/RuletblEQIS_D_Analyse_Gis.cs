...
{
    public class RuletblEQIS_D_Analyse_Gis : BaseRule
    {
        /// <summary>
        /// 功能描述    ：  取得海洋沉积物的综合评价[海洋沉积物断面（Gis）]
        /// 创建者      ：  du
        /// 创建日期    ：  2015-3-19
        /// 修改者      ：
        /// 修改日期    ：
                                             ...
                                             string StandardName, short iLevel,  string ItemCode, int iStatType,int iSpaceID,string DecCarry)
        {
            try
            {
                usp_tblEQIS_D_Report_DataStat_Gis usp = new usp_tblEQIS_D_Report_DataStat_Gis();
                usp.fldYear = iYear;
                usp.fldPhase = iPhase;
            ...
            {
                throw new GetByPKException("打开数据库连接失败", "RuletblEQIS_D_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
            catch (DBQueryException e)
            {
                throw new GetByPKException("执行Sql语句失败", "RuletblEQIS_D_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
            catch (Exception e)
            {
                throw new GetByPKException(e.Message, "RuletblEQIS_D_Analyse_Gis", "GetDataStatAnalyseGis", e.Message);
            }
        }
