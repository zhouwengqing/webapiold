...
namespace EMCControls_EMCMIS.Eqiw.Eqiw_g
{
    ...
    /// 地下水录入操作
    /// </summary>
    public class Eqiw_G_PointInputController : ApiController
    {
        ...
        public HttpResponseMessage ItemSave(eqia_rsavedata itemvalue)
        {
                ...
                if (itemvalue.BeginDate != null && itemvalue.EndDate != null)
                {
                    if (itemvalue.fldSTCode != null)
                    {
                        if (itemvalue.fldPCode != null)
                        {
                            ...
                            {
                                List<tblEQIW_G_Basedata_Pre> lstData = new List<tblEQIW_G_Basedata_Pre>();
                                tblEQIW_G_Basedata_Pre objData = new tblEQIW_G_Basedata_Pre();
                                ...
                                inputdate_new.fldObject = "eqiw_g";
                                ...
                                for (int i = 0; i < itemvalue.fldItemData.Count; i++)
                                {
                                    ...
                                    DataTable dataTable = rule.GetItem("地下水", itemvalue.fldItemData[i].itemcode);
                                    ...
                                    if (dataTable.Rows.Count > 0)
                                    {
                                        ...
                                        tblEQIW_G_Basedata_Pre objTmp = objData.Clone();
                                ...
                                RuletblEQIW_G_Basedata_Pre rule_basedata = new RuletblEQIW_G_Basedata_Pre();
                                bool issave = rule_basedata.InsertAll(lstData, inputdate_new);
                                if (issave)
                                {
                                    RuleWriteOperateLog rule_wol = new RuleWriteOperateLog();
                                    rule_wol.WriteLog(0, "录入地下水数据到临时表，断面：" + itemvalue.fldPCode +
            ...
            catch (InsertException ex)
            {
                PageException pagex = new PageException(int.Parse(itemvalue.fldUserID), ex.Message,
                    "Eqiw_gPointInputController", "ItemSave", "");
