...
namespace EMCControls_EMCMIS.Eqiw.Eqiw_DT
{
    ...
    /// 修改原因    ： 
    /// </summary>
    public class Eqiw_DT_SectionController : ApiController
    {
        ...
        /// <returns>返回区县饮用水测点信息</returns>
        [HttpGet]
        //
        public HttpResponseMessage GetEqiw_DT_Section(string STCode, int fldYear, short Level, int include)
        {
            string result = string.Empty;
            try
            {
                RuletblEQIW_DT_Section rule_point = new RuletblEQIW_DT_Section();
                IList<tblEQIW_DT_Section> list = rule_point.GetRCodeBySTCode(STCode, fldYear, Level, include);
        ...
        [HttpGet]
        //
        public HttpResponseMessage GetEqiw_DT_Section_V2(string STCode, string RCode, short Level, int include, int year)
        {
            string result = string.Empty;
            try
            {
                RuletblEQIW_DT_Section rule_point = new RuletblEQIW_DT_Section();
                IList<tblEQIW_DT_Section> list = rule_point.GetRSCode(STCode, RCode, Level, include, year);
                if (list != null && list.Count > 0)
                {
                    result = rule.JsonStr("ok", "", list);
                }
        ...
        [HttpGet]
        //
        public HttpResponseMessage GetEqiw_DT_RNameandRSName(string year, string stcode, string Level)
        {
            string result = string.Empty;
            try
            {
                ...
                {
                    RuletblEQIW_DT_Section rule_section = new RuletblEQIW_DT_Section();
                    IList<tblEQIW_DT_Section> list = rule_section.GetRSCodeandRCode(stcode, short.Parse(Level), year);
        ...
        [HttpGet]
        //
        public HttpResponseMessage GetW_DTSectionSelect(string where, int PageIndex, int PageSize, string flag)
        {
            string result = string.Empty;
            try
            {
                ...
                int count = 0;
                DataTable dt = rule.getpaging("vwEQIW_DT_Section_report", "*", "1=1" + where, PageIndex, PageSize, "fldSTCode", out count);
                string stcode = "[]";
                string year = "[]";
                string rername = "[]";
                string rersname = "[]";
                ...
                if (flag == "1")
                {
                    DataTable dt1 = rule.getdt("select * from vwEQIW_DT_Section_report where 1=1" + where);
                    //城市代码
                    var fldSTCode = (from x in dt1.AsEnumerable()
                                     select new tblEQIW_DT_Section
                                     {
