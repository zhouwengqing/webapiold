...
namespace EMCControls_EMCMIS.Eqia.Eqia_p
{
    ...
    /// 创建  人：周文卿
    /// </summary>
    public class Eqia_p_ItemInputController : ApiController
    {
        ...
        public HttpResponseMessage ItemSave(List<tbleqia_p_table> inputdata)
        {
            string returntext = "";
            try
            {
                ...
                {
                    List<tbaeqia_p_value> valueall = inputdata[i].valueall;
                    for (int j = 0; j < valueall.Count; j++)
                    {
                        ...
                        objData.fldType = short.Parse(inputdata[i].fldType);
                        #region 因子值的修改
                        decimal dValue = -1;
                ...
                if (issave)
                {
                    ...
                    rule_wol.WriteLog(0, "录入降水数据到临时表", "", int.Parse(inputdata[0].fldUserID), int.Parse(inputdata[0].fldCityID_Operate));
        ...
        public class tbleqia_p_table
        {
            ...
            /// 录入的详细信息
            /// </summary>
            public List<tbaeqia_p_value> valueall { get; set; }
